...
inline std::shared_ptr<Reader> deduce_reader(const std::string &filename)
{
    ...
    if ( input.m_root || input.m_remote) return   std::make_shared<ReaderPlugin>(filename,libHepMC3rootIO,std::string("newReaderRootTreefile"));
    HEPMC3_DEBUG(10, "deduce_reader: Attempt ProtobufIO for " << filename);
    if (input.m_protobuf) return std::make_shared<ReaderPlugin>(filename,libHepMC3protobufIO,std::string("newReaderprotobuffile"));
    ...
    if (input.m_asciiv3) return std::shared_ptr<Reader>((Reader*) ( new ReaderAscii(filename)));
    HEPMC3_DEBUG(10, "Attempt ReaderAsciiHepMC2 for " << filename);
    if ( input.m_iogenevent) return std::shared_ptr<Reader>((Reader*) ( new ReaderAsciiHepMC2(filename)));
    HEPMC3_DEBUG(10, "Attempt ReaderLHEF for " << filename);
    if ( input.m_lhef) return std::shared_ptr<Reader>((Reader*) ( new ReaderLHEF(filename)));
    HEPMC3_DEBUG(10, "Attempt ReaderHEPEVT for " << filename);
    if ( input.m_hepevt)  return std::shared_ptr<Reader>((Reader*) ( new ReaderHEPEVT(filename)));
